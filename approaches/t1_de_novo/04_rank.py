"""
Purpose: T_1 (de novo, DiffSBDD) — rank the docked candidates and build the shortlist.
Author: Mike Hallett (with Claude Code)
Date: 2026-07-28
Input: the latest D1 frame (post docking)
Output: D1 with rank / percentile / ligand-efficiency columns and a shortlist flag
...
ONE GROUP. seed-free by design, so its agreement with a seeded approach is not ancestral.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="T_1 (de novo, DiffSBDD): rank and shortlist.")
    ...
    df, frame_path = dio.latest_frame(EXPERIMENT, "t1")
    ...
    out = dio.write_full_frame(
        final, approach="t1", experiment=EXPERIMENT, stage="t1_rank",
    ...
    print(f"\nT_1 (de novo, DiffSBDD) ranking -> {out}")
