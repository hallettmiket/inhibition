...
SVG, NOT PNG. The depictions are vector, so they stay sharp when a reader zooms
into a crowded scaffold — which is the whole point of looking at the picture
rather than the string. Streamlit renders them through `st.markdown(...,
unsafe_allow_html=True)`; `st.image` does not handle SVG.
...
def grid(smiles_list, legends=None, *, per_row: int = 5,
    ...
    for s, lg in zip(smiles_list, legends):
        cells.append(
            ...
            f'font-size:0.75em">{svg(s, width=width, height=height)}'
